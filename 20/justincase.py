...
def show_game_advice():
    ...
    all_game_names = [game for games_list in game_data.values() for game in games_list]
    random_game_name = random.choice(all_game_names)

    # Here you can implement the logic to display game advice in the advice_frame
    advice_label.configure(text=f"Recommended game: {random_game_name}")
...
# Fetch rated games from the database
c.execute("SELECT name, rating, genre FROM games")
rated_games = c.fetchall()

# Create a dictionary to store rated games by genre
rated_games_by_genre = {}
for game_name, game_rating, game_genre in rated_games:
    if game_genre not in rated_games_by_genre:
        rated_games_by_genre[game_genre] = []
    rated_games_by_genre[game_genre].append((game_name, game_rating))

# Recommend games based on the rated games' genres
recommendations = {}
for genre, games in rated_games_by_genre.items():
    if genre in game_data:
        for game in game_data[genre]:
            for rated_game, rating in games:
                if rated_game.lower() in game.lower():
                    if genre not in recommendations:
                        recommendations[genre] = []
                    recommendations[genre].append((game, rating))

# Print recommendations
for genre, games in recommendations.items():
